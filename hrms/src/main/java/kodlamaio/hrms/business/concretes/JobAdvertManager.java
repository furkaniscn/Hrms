...
public class JobAdvertManager implements JobAdvertService {
	...
	public Result add(JobAdvert jobAdvert) {
		...
		this.jobAdvertDao.save(jobAdvert);
		return new SuccessResult("JobAdvert has been added.");
	}
	...
	public Result delete(int id) {
		...
		return new SuccessResult("JobAdvert has been deleted.");
	}

	@Override
	public Result changeOpenToClose(int id) {
		if (getById(id) == null) {
			return new ErrorResult("There is not such jobadvert.");
		}
		if (getById(id).getData().isOpen() == false) {
			return new ErrorResult("There jobadvert is already closed.");
		}

		JobAdvert jobAdvert = getById(id).getData();
		jobAdvert.setOpen(false);
		update(jobAdvert);
		return new SuccessResult("Jobadvert has been successfully closed.");
	}
